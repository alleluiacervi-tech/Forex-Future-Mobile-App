...
3. **Auth Routes** (`routes/auth.js`)
   - `POST /api/auth/register` - Register new user (starts email verification)
   - `POST /api/auth/email/verify` - Verify email (code)
   - `POST /api/auth/email/resend` - Resend verification code
   - `POST /api/auth/login` - Login user (requires verified email + active trial)
...
**Response (201):** (fields like `debugCode` may be present in development)
```json
{
  "user": {
    "id": "user_123",
  ...
  "debugCode": "123456",
  "debugExpiresAt": "2026-01-01T00:00:00.000Z"
}
```
...
**Errors:**
- `400` - Invalid/expired code or invalid email
- `429` - Too many attempts (rate limited)

### POST /api/auth/email/resend
Request a new verification code (response is generic to prevent account enumeration).
...
**Errors:**
- `400` - Validation error (bad email)
- `503` - Email verification temporarily unavailable (when email is required but not configured)
...
Request a password reset email (response is generic to prevent account enumeration).
...
**Errors:**
- `400` - Validation error (bad email)
- `503` - Password reset temporarily unavailable (when email is required but not configured)
...
const {
  ...
  resendEmailVerification,// Resend verification code
...
export default function AuthFlowExample() {
  const { register, verifyEmail, startTrial, login, isLoading } = useAuth();
  const [stage, setStage] = useState('register'); // 'register', 'verify', 'trial', 'login'
...
3. **Add OAuth** - Google, Apple sign-in integration
4. **Account Lockout** - Throttle + lockout on repeated failures
5. **Database Backup** - Regular backup of user data
